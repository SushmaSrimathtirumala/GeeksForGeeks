package Practice.Backtracking.Sudoku;
...
public class Sudoku {
    ...
    private static boolean solveSudoku(int[][] grid) {
        int[] unfilled = findNextUnfilled(grid);
        if (unfilled[0] == -1 && unfilled[1] == -1) {
            return true; // Base Condition, no unfilled items
        }
        for (int val = 1; val <= 9; val++) { // For loop, whenever we backtrack, check for nextRight case
    ...
    private static boolean isNotInBox(int val, int[][] grid, int[] unfilled) {
        int boxRowOrigin = unfilled[0] - unfilled[0] % 3;
        int boxColOrigin = unfilled[1] - unfilled[1] % 3;
