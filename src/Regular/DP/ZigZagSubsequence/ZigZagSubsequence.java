package Regular.DP.ZigZagSubsequence;
...
public class ZigZagSubsequence {
    ...
    private static int longestZigZagSubsequence(int[] arr) {
        ...
        for (int i = 1; i < arr.length; i++) {
            ...
            for (int j = 0; j < i; j++) {
                if (arr[j] < arr[i]) {
                    // We just found a decrement, so compare with last time increment with that current element + 1
        ...
        return Math.max(Arrays.stream(table[0]).max().getAsInt(), Arrays.stream(table[1]).max().getAsInt());
    }
}
